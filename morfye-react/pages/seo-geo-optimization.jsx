...
const featureIcons = [
  <SearchIcon />, <PenIcon />, <CogIcon />, <PinIcon />, <LinkIcon />, <BarChartIcon />,
  <TargetIcon />, <RocketIcon />, <LightbulbIcon />, <ChartIcon />, <GlobeIcon />, <DiamondIcon />
]

export default function SeoGeoOptimization() {
  const { t } = useLanguage()
  ...
  const hero = t('seoGeo.hero')
  const whatIsSeo = t('seo.whatIsSeo')
  const whatIsGeo = t('geo.whatIsGeo')
  const features = t('seo.features').concat(t('geo.features'))
  const steps = t('seo.steps').concat(t('geo.steps'))
  const faqs = t('seo.faqs').concat(t('geo.faqs'))
  const cta = t('seoGeo.cta')

  return (
    <ServicePageLayout
      title={t('seoGeo.meta.title')}
      description={t('seoGeo.meta.description')}
      navLinks={t('seoGeo.navLinks')}
    ...
    >
      <div className="geo-page">
        ...
        {/* WHAT IS SEO & GEO */}
        <section ref={explainerRef} className="geo-explainer" id="what-is-seo-geo">
          ...
          <div className="geo-explainer-grid" style={{ marginTop: '80px' }}>
            <div className="geo-explainer-side">
              <div className="geo-explainer-number">02</div>
        ...
        <section ref={featuresRef} className="geo-features" id="our-services">
          <h2 className="geo-features-heading">{t('seoGeo.featuresHeading')}</h2>
          <div className="geo-features-grid">
            {features.map((f, i) => (
              <motion.div
                ...
                transition={{ duration: 0.6, delay: (i % 6) * 0.1, ease: 'easeOut' }}
                whileHover={{ y: -8 }}
              >
